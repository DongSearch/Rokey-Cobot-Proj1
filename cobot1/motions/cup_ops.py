...
def get_shared_cup_positions():
    ...
    delta_y = 7  # 컵을 들 때 y축 이동량 (필요에 따라 조정)
    ...
    pos5 = posx([420.64+delta_x, 120.19+delta_y, 201.16+delta_z, 90.72, -90.54, 174.76]) # closer to cup
    pos6 = posx([417.15+delta_x, 120.95+delta_y, 204.58+delta_z, 90.11, -89.97, -154.06])# pour finish
...
def cup1():
    ...
    movel(pos6, vel=VELOCITY-50, acc=ACC-50) # pour finish
...
def cup2():
    ...
    movel(pos6, vel=VELOCITY-50, acc=ACC-50) 
    
    movel(pos3, vel=VELOCITY, acc=ACC) 
    movel(pos2, vel=VELOCITY, acc=ACC) # 2 back
    movel(pos1, vel=VELOCITY, acc=ACC) 
    release()
    movel(pos2, vel=VELOCITY, acc=ACC) 
...
def cup3():
    ...
    pos2 = posx([600, -53.02, 470.75, 90.81, -90.00, 89.99])  # 3 back
    pos3, pos4, pos5, pos6 = get_shared_cup_positions()
    ...
    movel(pos3, vel=VELOCITY, acc=ACC) 
    movel(pos2, vel=VELOCITY, acc=ACC) # 3 back
    movel(pos1, vel=VELOCITY, acc=ACC) 
    release()
    movel(pos2, vel=VELOCITY, acc=ACC) 